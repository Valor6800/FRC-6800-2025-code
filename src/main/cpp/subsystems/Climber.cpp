...
Climber::Climber(frc::TimedRobot *_robot, valor::CANdleSensor *_leds)
    : valor::BaseSubsystem(_robot, "Climber"), climbMotors(nullptr),
      currentSensor(_robot, "Climber"),
      climbCancoder(new ctre::phoenix6::hardware::CANcoder(CANIDs::CLIMBER_CAN,
                                                           "baseCAN")),
      leds(_leds) {
...
void Climber::init() {
  ...
  climbMotors = new valor::PhoenixController(
      valor::PhoenixControllerType::KRAKEN_X44_FOC, CANIDs::CLIMBER_LEAD,
      valor::NeutralMode::Brake, Constants::Climber::climbMotorInverted(),
      "baseCAN");
  ...
  // climbMotors->setupCANCoder(CANIDs::CLIMBER_CAN,
  // Constants::Climber::magnetOffset(), true, "baseCAN", 1_tr); //0.5022
  ...
  climbMotors->setCurrentLimits(units::ampere_t{100}, units::ampere_t{60},
  ...
  climbPID.maxAcceleration = climbMotors->getMaxMechSpeed() /
                             Constants::Climber::maxVelocityRampTime();

  climbRetractPID.maxVelocity = climbMotors->getMaxMechSpeed();
  climbRetractPID.maxAcceleration = climbMotors->getMaxMechSpeed() /
                                    Constants::Climber::maxVelocityRampTime();
  ...
  // climbMotors->setupFollower(CANIDs::CLIMBER_FOLLOW,
  ...
  climbMotors->setVoltageLimits(units::volt_t(0.0), units::volt_t(12.0));
  ...
  climbMotors->setEncoderPosition(
      climbCancoder->GetAbsolutePosition().GetValue());
}
...
void Climber::analyzeDashboard() {
  leds->setLED(LEDConstants::LED_POS_CLIMBER,
               valor::CANdleSensor::cancoderMagnetHealthGetter(climbCancoder));
  if (state.hasClimbed == true) {
    leds->setColorAll(CANdleSensor::GREEN,
                      CANdleSensor::Priority::PRIORITY_CLIMBER);
  } else if (state.climbState == CLIMB_STATE::DEPLOYED) {
    leds->setColorAll(CANdleSensor::RED,
                      CANdleSensor::Priority::PRIORITY_CLIMBER);
  } else {
    leds->resetPriority();
  }
...
void Climber::assignOutputs() {
  ...
  //  if (state.climbState == CLIMB_STATE::DEPLOYED || state.stabState ==
  //  STABBY_STATE::CRABBING) {
...
frc2::CommandPtr Climber::pitSequenceStage(CLIMB_STATE climbState) {
  ...
  if (climbState == RETRACTED)
    climbPos = RETRACTED_POS;
  else if (climbState == DEPLOYED)
    climbPos = DEPLOYED_POS;
  return frc2::cmd::Parallel(
      frc2::cmd::Wait(5_s),
      frc2::FunctionalCommand{[this, climbState] {
...
frc2::CommandPtr Climber::pitSequence() {
  return frc2::cmd::Sequence(pitSequenceStage(STOW), pitSequenceStage(DEPLOYED),
...
void Climber::InitSendable(wpi::SendableBuilder &builder) {
  ...
  builder.AddIntegerProperty(
      "Climb State", [this] { return state.climbState; }, nullptr);
  // builder.AddIntegerProperty(
  //     "Stabby State",
