...
#define VIABLE_ELEVATOR_DISTANCE                                               \
  1.8_m // consider the offset of the canrange from the front of robot which is
        // 8 inches
...
Scorer::Scorer(frc::TimedRobot *_robot, Drivetrain *_drivetrain,
               Climber *_climber, valor::CANdleSensor *_leds)
    : valor::BaseSubsystem(_robot, "Scorer"),
      hallEffectDebounceSensor(_robot, "HallEffectDebounce"),
      candi(CANIDs::HALL_EFFECT, "baseCAN"),
      elevatorMotor(new valor::PhoenixController(
          valor::PhoenixControllerType::KRAKEN_X60, CANIDs::ELEV_WHEEL,
          valor::NeutralMode::Brake, elevatorMotorInverted(), "baseCAN")),
      scorerMotor(new valor::PhoenixController(
          Constants::getScorerMotorType(), CANIDs::SCORER_WHEEL,
          valor::NeutralMode::Brake, scorerMotorInverted(), "baseCAN")),
      scorerPivotMotor(new valor::PhoenixController(
          Constants::getScorerPivotMotorType(), CANIDs::SCORER_PIVOT_MOTOR,
          valor::NeutralMode::Brake, scorerPivotInverted(), "baseCAN")),
      funnelMotor(new valor::PhoenixController(
          Constants::getFunnelMotorType(), CANIDs::FUNNEL,
          valor::NeutralMode::Coast, funnelInverted(), "baseCAN")),
      scorerStagingSensor(_robot, "Scorer Staging Sensor",
                          CANIDs::STAGING_LIDAR_SENSOR, "baseCAN", -1_mm),
  ...
  pathplanner::NamedCommands::registerCommand(
      ...
      std::move(frc2::SequentialCommandGroup(
                    ...
                    frc2::WaitCommand(5_s), frc2::InstantCommand([this]() {
  ...
  pathplanner::NamedCommands::registerCommand(
      "AlignLeft",
      std::move(frc2::SequentialCommandGroup(frc2::FunctionalCommand(
                    [&] { // on begin
                      drivetrain->state.startTimestamp =
                    ...
                    [&] { // on execute
                      ...
                      // frc::ChassisSpeeds speeds = frc::ChassisSpeeds{0.5_mps,
                      // 0.0_mps, 0.0_rad_per_s}; drive(0.5_mps, 0.0_mps,
                      // 0.0_rad_per_s, true);
                    },
                    [&](bool) { // on end
                    ...
                    [&] { // is Finished
                      ...
                      return (!scorerStagingSensor.isTriggered()) &&
                             drivetrain->withinXRange(
                                 (units::meter_t)table->GetNumber(
                                     "Viable Elevator Distance (m)",
                                     VIABLE_ELEVATOR_DISTANCE.value()));
                    },
                    {})))
          .ToPtr());
  ...
  table->PutNumber("Viable Elevator Distance (m)",
                   VIABLE_ELEVATOR_DISTANCE.value());

  init();

  visualizerStage1 = nt::NetworkTableInstance::GetDefault()
                         .GetStructTopic<frc::Pose3d>(
                             "LiveWindow/BaseSubsystem/Scorer/Stage1Height")
                         .Publish();
  visualizerStage2 = nt::NetworkTableInstance::GetDefault()
                         .GetStructTopic<frc::Pose3d>(
                             "LiveWindow/BaseSubsystem/Scorer/Stage2Height")
                         .Publish();
  visualizerStage3 = nt::NetworkTableInstance::GetDefault()
                         .GetStructTopic<frc::Pose3d>(
                             "LiveWindow/BaseSubsystem/Scorer/Stage3Height")
                         .Publish();
}

frc2::CommandPtr Scorer::scorerPitSequenceStage(GAME_PIECE gamePiece,
                                                ELEVATOR_STATE elevState,
                                                int offset, int numLeds) {
  return frc2::cmd::Deadline(
      ...
      frc2::FunctionalCommand{
          [this, elevState, gamePiece] {
            ...
            if (gamePiece == ALGEE)
              msg += "algae ";
            else if (gamePiece == CORAL)
              msg += "coral ";
            if (elevState == STOWED)
              msg += "stowed";
            else if (elevState == HP)
              msg += "human player";
            else if (elevState == ONE)
              msg += "L1";
            else if (elevState == TWO)
              msg += "L2";
            else if (elevState == THREE)
              msg += "L3";
            else if (elevState == FOUR)
              msg += "L4";
          ...
          [this] {
            units::inch_t currentPos =
                convertToMechSpace(elevatorMotor->getPosition());
            units::inch_t targetPos =
                positionMap[state.gamePiece][state.elevState];
            elevatorPositionSuccess.Set(
                units::math::abs(targetPos - currentPos) < 0.25_in);
            elevatorPositionFail.Set(!elevatorPositionSuccess.Get());
          },
          [this, offset, numLeds](bool) {
            int color = elevatorPositionSuccess.Get()
                            ? valor::CANdleSensor::GREEN
                            : valor::CANdleSensor::RED;
...
bool Scorer::hallEffectSensorActive() { return candi.GetS1Closed().GetValue(); }

bool Scorer::cancoderSensorBad() {
  return (elevatorMotor->GetFault_BadMagnet() ||
          (elevatorMotor->getMagnetHealth() ==
...
void Scorer::init() {
  ...
  candiConfig.DigitalInputs.S1CloseState =
      ctre::phoenix6::signals::S1CloseStateValue::CloseWhenLow;
  candiConfig.DigitalInputs.S1FloatState =
      ctre::phoenix6::signals::S1FloatStateValue::FloatDetect;
  ...
  elevatorMotor->setGearRatios(ELEVATOR_MOTOR_TO_SENSOR,
                               ELEVATOR_SENSOR_TO_MECH);
  elevatorMotor->enableFOC(true);
  elevatorMotor->setForwardLimit(ELEVATOR_FORWARD_LIMIT);
  elevatorMotor->setupReverseHardwareLimit(
      CANIDs::HALL_EFFECT,
  ...
  elevatorPID.maxAcceleration = elevatorMotor->getMaxMechSpeed() /
                                Constants::Scorer::getElevMaxVelRampTime();
  elevatorMotor->setPIDF(elevatorPID);
  elevatorMotor->setupCANCoder(
      CANIDs::ELEVATOR_CAN, Constants::getElevatorMagnetOffset(),
      Constants::Scorer::isElevatorClockwise(), "baseCAN",
      Constants::Scorer::getElevatorAbsoluteRange());
  elevatorMotor->setCurrentLimits(units::ampere_t{120}, units::ampere_t{60},
                                  units::ampere_t{45}, units::second_t{0.5},
                                  true);
  ...
  scorerMotor->setCurrentLimits(units::ampere_t{100}, units::ampere_t{60},
                                units::ampere_t{45}, units::second_t{0.5},
                                true);

  valor::PIDF scorerPID = Constants::Scorer::getScorerPIDF();
  scorerPID.maxVelocity = scorerMotor->getMaxMechSpeed();
  scorerPID.maxAcceleration = scorerMotor->getMaxMechSpeed() /
                              Constants::Scorer::getScorerMaxVelRampTime();
  ...
  pivotPID.maxAcceleration = units::turns_per_second_squared_t(
      3.45); // scorerPivotMotor->getMaxMechSpeed() / 1_s;
  scorerPivotMotor->setPIDF(pivotPID);
  scorerPivotMotor->setupCANCoder(
      CANIDs::SCORER_PIVOT_CAN, scorerPivotMagnetOffset(),
  ...
  funnelMotor->setCurrentLimits(units::ampere_t{100}, units::ampere_t{60},
                                units::ampere_t{45}, units::second_t{0.5},
                                true);
  ...
  hallEffectDebounceSensor.setGetter(
    ...
    if (frc::DriverStation::IsTest()) {
      std::cout
          ...
          << -(Constants::getElevatorMagnetOffset() -
               elevatorMotor->getCANCoder()->GetAbsolutePosition().GetValue())
                  .value()
          << std::endl;
    }
  });
  ...
  coralCurrentSensor.setGetter(
      [this]() { return scorerMotor->getCurrent().to<double>(); });
  coralCurrentSensor.setSpikeCallback([this]() {
    if (scorerPivotMotor->getSpeed() < MINIMUM_PIVOT_SPEED &&
        state.gamePiece == GAME_PIECE::CORAL && state.intaking) {
      state.hasCoral = true;
      coralCurrentSensor.reset();
    }
  });
  ...
  // Must be at the end of init() because the CANdi has to be setup before
  // reading
  if (cancoderSensorBad()) {
  ...
  if (state.elevState == ELEVATOR_STATE::ONE &&
    ...
    if (driverGamepad->GetLeftBumperButton()) {
      state.scopedState = MANUAL_SCOPE;
               ...
               driverGamepad->rightTriggerActive()) {
...
void Scorer::analyzeDashboard() {
  ...
  int elevatorHealthLED =
      std::abs(absSensorCorrect.value()) < ELEVATOR_HEALTH_LIMIT.value()
          ? valor::CANdleSensor::GREEN
          : valor::CANdleSensor::RED;
  leds->setLED(LEDConstants::LED_POS_ELEVATOR_NOT_ZERO, elevatorHealthLED);

  if (candi.GetS1State().GetValue() ==
      ctre::phoenix6::signals::S1StateValue::Floating) {
  ...
  if (state.elevState == ELEVATOR_STATE::FOUR &&
      drivetrain->getXConstraints().maxVelocity !=
          drivetrain->x_constraints_l4.maxVelocity) {
    drivetrain->setXConstraints(drivetrain->x_constraints_l4);
  } else if ((state.elevState == ELEVATOR_STATE::ONE ||
              state.elevState == ELEVATOR_STATE::TWO ||
              state.elevState == ELEVATOR_STATE::THREE) &&
             drivetrain->getXConstraints().maxVelocity !=
                 drivetrain->x_constraints.maxVelocity) {
  ...
  units::meter_t elevatorSetpoint =
      positionMap[state.gamePiece][state.elevState];

  bool isStopped = drivetrain->isSpeedStopped();
  units::meter_t distanceFromReef = drivetrain->lidarDistance();
  state.shootOverCoral =
      false; // isStopped && (MIN_DUNK_DISTANCE_OVER_CORAL<= distanceFromReef &&
             // distanceFromReef <= MAX_DUNK_DISTANCE_OVER_CORAL);
  if (state.shootOverCoral) {
    elevatorSetpoint += DUNK_OFFSET_OVER_CORAL;
  }

  units::meter_t elevatorError = units::math::fabs(
      convertToMechSpace(elevatorMotor->getPosition()) - elevatorSetpoint);
  elevatorWithinThreshold = elevatorError.value() <
                            table->GetNumber("Elevator Threshold (m)",
                                             VIABLE_ELEVATOR_THRESHOLD.value());

  if ((state.autoDunkEnabled && !disableAutoDunk) &&
      state.scopedState == SCOPED_STATE::SCOPED && elevatorWithinThreshold &&
      state.gamePiece == GAME_PIECE::CORAL &&
      (state.elevState == TWO || state.elevState == THREE ||
       state.elevState == FOUR) &&
      drivetrain->getAutoDunkAcceptance().all()) {
    state.scoringState = SCORE_STATE::SCORING;
  }

  int botColor = state.gamePiece == GAME_PIECE::CORAL
                     ? valor::CANdleSensor::VALOR_GOLD
                     : valor::CANdleSensor::VALOR_PURPLE;
  int midTopColor = state.scoringState == SCORE_STATE::SCORING
                        ? valor::CANdleSensor::BLUE
                        : (drivetrain->state.alignToTarget
                               ? valor::CANdleSensor::RED
                               : (scorerStagingSensor.isTriggered() ||
                                          state.hasAlgae || state.hasCoral
                                      ? valor::CANdleSensor::GREEN
                                      : valor::CANdleSensor::OFF));

  if (!frc::DriverStation::IsTestEnabled()) {
    leds->setColor(0, botColor, valor::CANdleSensor::Priority::PRIORITY_SCORER);
    leds->setColor(1, midTopColor,
                   valor::CANdleSensor::Priority::PRIORITY_SCORER);
    leds->setColor(2, midTopColor,
                   valor::CANdleSensor::Priority::PRIORITY_SCORER);
    leds->setColor(3, midTopColor,
                   valor::CANdleSensor::Priority::PRIORITY_SCORER);
    leds->setColor(4, midTopColor,
                   valor::CANdleSensor::Priority::PRIORITY_SCORER);
  ...
  if ((state.elevState == TWO || state.elevState == THREE) &&
      state.gamePiece == GAME_PIECE::ALGEE && !state.intaking &&
      (state.scopedState == SCOPED || state.scopedState == MANUAL_SCOPE)) {
    if (tagID >= 6 && tagID <= 11) {
      state.elevState =
          (tagID % 2 == 1) ? ELEVATOR_STATE::THREE : ELEVATOR_STATE::TWO;
    } else if (tagID >= 17 && tagID <= 22) {
      state.elevState =
          (tagID % 2 == 0) ? ELEVATOR_STATE::THREE : ELEVATOR_STATE::TWO;
    }
  }
  ...
  visualizerStage1.Set(
      frc::Pose3d{0_m, 0_m, convertToMechSpace(height), frc::Rotation3d()});
  visualizerStage2.Set(
      frc::Pose3d{0_m, 0_m, convertToMechSpace(height * 2), frc::Rotation3d()});
  visualizerStage3.Set(
      frc::Pose3d{0_m, 0_m, convertToMechSpace(height * 3), frc::Rotation3d()});
}

void Scorer::setScopedState(SCOPED_STATE st) { state.scopedState = st; }

units::meter_t Scorer::convertToMechSpace(units::turn_t turns) {
  return units::meter_t{turns * units::meter_t{PULLEY_CIRCUMFERENCE * M_PI} /
                        1_tr} +
         ELEVATOR_OFFSET;
}

units::turn_t Scorer::convertToMotorSpace(units::meter_t meters) {
  return (meters - ELEVATOR_OFFSET) /
         units::meter_t{PULLEY_CIRCUMFERENCE * M_PI} * 1_tr;
}
...
void Scorer::assignOutputs() {
    ...
    if (state.gamePiece == GAME_PIECE::ALGEE) {
      if (state.intaking) {
        if (state.hasAlgae && !state.intaking) {
          scorerPivotMotor->setPosition(
              getPivotPositionMap()[PIVOT_STATE::CARRY]);
        } else {
          scorerPivotMotor->setPosition(
              getPivotPositionMap()[PIVOT_STATE::GROUND]);
        }
      } else if (state.elevState == ELEVATOR_STATE::FOUR &&
                 state.gamePiece == GAME_PIECE::ALGEE) {
        if (units::math::fabs(convertToMechSpace(elevatorMotor->getPosition()) -
                              positionMap[state.gamePiece][state.elevState]) <
            0.1_m) {
          scorerPivotMotor->setPosition(
              getPivotPositionMap()[PIVOT_STATE::PICK]);
        } else {
          scorerPivotMotor->setPosition(
              getPivotPositionMap()[PIVOT_STATE::CARRY]);
        }
      } else if (state.scopedState == SCOPED_STATE::SCOPED ||
                 state.scopedState == SCOPED_STATE::MANUAL_SCOPE) {
        if (state.elevState == ELEVATOR_STATE::TWO ||
            state.elevState == ELEVATOR_STATE::THREE ||
            state.elevState == ELEVATOR_STATE::FOUR) {
          scorerPivotMotor->setPosition(
              getPivotPositionMap()[PIVOT_STATE::PICK]);
          ...
          scorerPivotMotor->setPosition(
              getPivotPositionMap()[PIVOT_STATE::PRESCORE]);
        }
      ...
      if (state.intaking) {
        scorerPivotMotor->setPosition(
            getPivotPositionMap()[PIVOT_STATE::CORAL_GROUND]);
      } else {
        if (state.elevState == ELEVATOR_STATE::ONE) {
          scorerPivotMotor->setPosition(
              getPivotPositionMap()[PIVOT_STATE::PICK]);
          ...
          scorerPivotMotor->setPosition(
              getPivotPositionMap()[PIVOT_STATE::CORAL_STOW]);
        }
      }
    }
  } else {
  ...
  if (state.elevState == ELEVATOR_STATE::MANUAL) {
    elevatorMotor->setPower(
        state.manualSpeed +
        units::volt_t{Constants::Scorer::getElevatorPIDF().aFF});
  } else {
    if (state.intaking) {
      if (state.hasAlgae) {
        state.targetHeight =
            positionMap.at(state.gamePiece).at(ELEVATOR_STATE::STOWED);
      } else {
        state.targetHeight =
            positionMap.at(state.gamePiece).at(ELEVATOR_STATE::FLOOR);
      }
    } else if ((state.scopedState == SCOPED &&
                (state.gamePiece == GAME_PIECE::ALGEE ||
                 (state.gamePiece == GAME_PIECE::CORAL &&
                  drivetrain->withinXRange((units::meter_t)table->GetNumber(
                      "Viable Elevator Distance (m)",
                      VIABLE_ELEVATOR_DISTANCE.value()))))) ||
               state.scopedState == MANUAL_SCOPE) {
  ...
  if (!scorerStagingSensor.isTriggered() &&
      state.gamePiece == GAME_PIECE::CORAL && !state.intaking &&
      (convertToMechSpace(elevatorMotor->getPosition()) <
           positionMap.at(CORAL).at(HP) + ELEVATOR_THRESHOLD_FOR_FUNNEL ||
       hallEffectSensorActive()) &&
      state.elevState != ELEVATOR_STATE::ONE &&
      scorerPivotMotor->getPosition() <
          getPivotPositionMap().at(PIVOT_STATE::CORAL_STOW) +
              PIVOT_THRESHOLD_FOR_FUNNEL &&
...
void Scorer::InitSendable(wpi::SendableBuilder &builder) {
  ...
  builder.AddDoubleProperty(
      ...
      [this] { return units::inch_t{state.targetHeight}.value(); }, nullptr);
  builder.AddDoubleProperty(
      "Actual Position: Elevator (in)",
      [this] {
        return units::inch_t{convertToMechSpace(elevatorMotor->getPosition())}
            .value();
      },
      nullptr);
  builder.AddDoubleProperty(
      "State: Scoring", [this] { return state.scoringState; }, nullptr);
  builder.AddIntegerProperty(
      "State: Gamepiece", [this] { return state.gamePiece; }, nullptr);
  builder.AddIntegerProperty(
      "State: Elevator Level", [this] { return state.elevState; }, nullptr);
  builder.AddBooleanProperty(
      "State: Scoped", [this] { return state.scopedState; }, nullptr);
  builder.AddDoubleProperty(
      "Desired Speed: Scorer",
      [this] {
        auto speed = scoringSpeedMap.find(state.elevState);
        if (speed == scoringSpeedMap.end())
          return 0.0;
  ...
  builder.AddDoubleProperty(
      ...
      [this] { return state.manualSpeed.to<double>(); }, nullptr);
  builder.AddBooleanProperty(
      "Has Algae", [this] { return state.hasAlgae; }, nullptr);
  builder.AddBooleanProperty(
      "Has Coral", [this] { return state.hasCoral; }, nullptr);
  builder.AddBooleanProperty(
      "CORAL", [this] { return state.gamePiece == GAME_PIECE::CORAL; },
      nullptr);
  builder.AddBooleanProperty(
      "ALGAE", [this] { return state.gamePiece == GAME_PIECE::ALGEE; },
      nullptr);

  builder.AddBooleanProperty(
      "ZEROED?", [this] { return state.hasZeroed; }, nullptr);

  builder.AddBooleanProperty(
      "ABSOLUTE POSITION NO OFFSET?",
      [this] { return absSensorCorrect.value(); }, nullptr);
  builder.AddBooleanArrayProperty(
      "Auto Dunk Acceptance",
      [this] {
        std::bitset<5> drivetrainAcceptances =
            drivetrain->getAutoDunkAcceptance();
        std::vector<int> gates = {
            state.autoDunkEnabled &&
                !table->GetBoolean("Auto Dunk Disabled", false), // 0
