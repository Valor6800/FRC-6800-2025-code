...
const units::turns_per_second_squared_t NEO_PIDF_KA(
    130.0); // RPS/S acceleration (6.5/130 = 0.05 seconds to max speed)
const units::turns_per_second_cubed_t NEO_PIDF_KJ(
    650.0); // RPS/S^2 jerk (4000/40000 = 0.1 seconds to max acceleration)
...
NeoController::NeoController(valor::NeoControllerType controllerType, int canID,
                             valor::NeutralMode _mode, bool _inverted,
                             std::string canbus)
    : BaseController(new rev::spark::SparkMax(
                         canID, rev::spark::SparkMax::MotorType::kBrushless),
                     _inverted, _mode,
                     getNeoControllerMotorSpeed(controllerType)),
      cancoder(nullptr), pidController(motor->GetClosedLoopController()) {
  init();
}
...
void NeoController::init() {
  ...
  wpi::SendableRegistry::AddLW(this, "NeoController",
                               "ID " + std::to_string(0));
}

void NeoController::setupCANCoder(int deviceId, units::turn_t offset,
                                  bool clockwise, std::string canbus,
                                  units::turn_t absoluteRange,
...
void NeoController::setForwardLimit(units::turn_t forward,
                                    bool saveImmediately) {}

void NeoController::setReverseLimit(units::turn_t reverse,
                                    bool saveImmediately) {}

void NeoController::setCurrentLimits(units::ampere_t statorCurrentLimit,
...
void NeoController::setGearRatios(double _rotorToSensor, double _sensorToMech,
                                  bool saveImmediately) {
  rotorToSensor = _rotorToSensor;
  sensorToMech = _sensorToMech;
}

units::ampere_t NeoController::getCurrent() { return units::ampere_t{0}; }

/**
 * Output is in mechanism rotations!
 */
units::turn_t NeoController::getPosition() { return 0_tr; }

/**
 * Output is in mechanism rotations!
 */
units::turns_per_second_t NeoController::getSpeed() { return 0_tps; }
...
void NeoController::setOpenLoopRamp(units::second_t time,
                                    bool saveImmediately) {}
