...
VisionSensor::VisionSensor(frc::TimedRobot *robot, const char *name,
                           frc::Pose3d _cameraPose)
    : BaseSensor(robot, name), cameraPose(_cameraPose),
...
void VisionSensor::setCameraPose(frc::Pose3d camPose) {
  ...
  double roll =
      camPose.Rotation().X().convert<units::angle::degrees>().to<double>();
  double pitch =
      camPose.Rotation().Y().convert<units::angle::degrees>().to<double>();
  double yaw =
      camPose.Rotation().Z().convert<units::angle::degrees>().to<double>();

  std::array<double, 6> camPosition =
      std::array<double, 6>{x, y, z, roll, pitch, yaw};

  limeTable->PutNumberArray("camerapose_robotspace_set", camPosition);
}

const char *VisionSensor::getName() { return BaseSensor::sensorName; }
...
bool VisionSensor::hasTarget() { return limeTable != nullptr && tv == 1; }

units::millisecond_t VisionSensor::getTotalLatency() {
  return (units::millisecond_t)(limeTable->GetNumber("cl", 0.0) +
                                limeTable->GetNumber("tl", 0.0));
}

units::velocity::meters_per_second_t
VisionSensor::getError(int pipe, double kPLimeLight) {
  if (limeTable != nullptr && hasTarget()) {
    double normalizedTx = tx / KLIMELIGHT;
    return units::velocity::meters_per_second_t(
        ((std::fabs(normalizedTx) <= 1 ? normalizedTx
                                       : std::copysignf(1.0, normalizedTx)) *
         kPLimeLight));
  }
...
void VisionSensor::calculate() {
  ...
  std::vector<double> hw =
      limeTable->GetNumberArray("hw", std::array<double, 4>());
...
void VisionSensor::InitSendable(wpi::SendableBuilder &builder) {
  builder.AddDoubleProperty(
      "totalLatency", [this] { return getTotalLatency().to<double>(); },
      nullptr);
