...
CANdleSensor::CANdleSensor(frc::TimedRobot *_robot, int _ledCount,
                           int _segments, int _canID, std::string _canbus)
    : BaseSensor(_robot,
                 std::string("ID ").append(std::to_string(_canID)).c_str()),
      candle(_canID, _canbus), ledCount(_ledCount), segments(_segments) {
  ...
  for (int i = 0; i < segments + 1; i++) {
    ...
    if (i == 0) {
      ...
      if ((i % 2 == 0 && i != 0 && remainderLEDCount > 0) ||
          (i == segments && remainderLEDCount > 0)) {
...
int CANdleSensor::cancoderMagnetHealthGetter(
  ...
  if (cancoder && cancoder->IsConnected()) {
    ...
    if (value == MagnetHealthValue::Magnet_Green)
      return valor::CANdleSensor::GREEN;
    else if (value == MagnetHealthValue::Magnet_Orange)
      return valor::CANdleSensor::ORANGE;
  }
...
void CANdleSensor::setLED(int led, RGBColor rgb) {
  if (led <= 8) {
    ...
    rgb.green *= 0.4;
    rgb.blue *= 0.4;
    candle.SetLEDs(rgb.red, rgb.green, rgb.blue, 0, led, 1);
  } else {
    candle.SetLEDs(rgb.green, rgb.red, rgb.blue, 0, led, 1);
  }
}

void CANdleSensor::setLED(int led, int color) { setLED(led, toRGB(color)); }

void CANdleSensor::setColor(int segment, RGBColor rgb) {
  segment++;
  if (segment >= static_cast<int>(segmentMap.size()))
    return;
...
void CANdleSensor::setAnimation(AnimationType animation, RGBColor color,
                                double speed) {
  clearAnimation(0);
  setAnimation(&allSegments, animation, color, speed);
}

void CANdleSensor::setAnimation(int segment, AnimationType animation,
                                RGBColor color, double speed) {
  segment++;
  if (segment >= static_cast<int>(segmentMap.size()))
    return;
...
void CANdleSensor::resetPriority() { currentPriority = PRIORITY_DEFAULT; }

void CANdleSensor::setAnimation(CANdleSensor::SegmentSettings *segment,
                                AnimationType animation, RGBColor color,
                                double speed) {
...
void CANdleSensor::clearAnimation() { clearAnimation(-1); }
...
void CANdleSensor::calculate() {
  for (auto segment : segmentMap) {
    ...
    if (segmentMap[segment.first].recentlyChanged) {
      candle.SetLEDs(segmentMap[segment.first].currentColor.green,
                     segmentMap[segment.first].currentColor.red,
                     segmentMap[segment.first].currentColor.blue, 0,
...
void CANdleSensor::InitSendable(wpi::SendableBuilder &builder) {
  ...
  builder.AddDoubleProperty(
      "Max Animations",
      [this] { return candle.GetMaxSimultaneousAnimationCount(); }, nullptr);
}
