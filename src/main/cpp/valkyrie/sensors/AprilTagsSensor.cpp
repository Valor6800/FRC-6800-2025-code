...
AprilTagsSensor::AprilTagsSensor(frc::TimedRobot *robot, const char *name,
...
frc::Pose3d AprilTagsSensor::getGlobalPose() {
  ...
  std::vector<double> botPose =
      limeTable->GetNumberArray("botpose_wpiblue", std::span<double>());

  std::vector<double> botToTargetPose = limeTable->GetNumberArray(
      "botpose_targetspace", std::span<const double>());

  if (botPose.size() == 0 || botToTargetPose.size() == 0) {
    distance = 0_m;
    return frc::Pose3d();
  }

  if (botToTargetPose.size() == 6)
    distance = units::meter_t(sqrtf(powf(botToTargetPose[0], 2) +
                                    powf(botToTargetPose[1], 2) +
                                    powf(botToTargetPose[2], 2)));

  return frc::Pose3d((units::meter_t)botPose[0], (units::meter_t)botPose[1],
                     (units::meter_t)botPose[2],
                     frc::Rotation3d((units::degree_t)botPose[3],
...
frc::Pose3d AprilTagsSensor::getPoseFromAprilTag() {
  ...
  if (botToTargetPose.size() == 0)
    return frc::Pose3d();

  return frc::Pose3d((units::meter_t)botToTargetPose[0],
                     (units::meter_t)botToTargetPose[1],
                     (units::meter_t)botToTargetPose[2],
                     frc::Rotation3d((units::degree_t)botToTargetPose[3],
...
void AprilTagsSensor::applyVisionMeasurement(
    ...
    double doubtX, double doubtY, double doubtRot) {
  if (!hasTarget())
    return;
  dp = DP;
  vp = VP;
  ...
  // limeTable->GetNumberArray("botpose_targetspace", std::span<const
  // double>()); if (botToTargetPose.size() == 6) distance =
  // units::meter_t(sqrtf(powf(botToTargetPose[0], 2) + powf(botToTargetPose[1],
  // 2))); else distance = 0_m; return;

  double newDoubtX =
      doubtX + (distance.to<double>() * dp) + (vp * speed.to<double>());
  double newDoubtY =
      doubtY + (distance.to<double>() * dp) + (vp * speed.to<double>());
  double newDoubtRot = doubtRot + (distance.to<double>() * dp) +
                       (vp * speed.to<double>()) +
                       (avp * std::fabs(angular_velocity.value()));
  ...
  frc::Pose2d tGone =
      frc::Pose2d{currState.ToPose2d().X(), currState.ToPose2d().Y(), newAngle};
  ...
  estimator->AddVisionMeasurement(tGone,
...
AprilTagsSensor::getMegaTagPose2(AprilTagsSensor::Orientation orient) {
  ...
  limeTable->PutNumberArray("robot_orientation_set",
                            std::vector<double>{orient.yaw.to<double>(),
                                                orient.yawVel.to<double>(), 0,
                                                0, 0, 0});

  std::vector<double> mt2Array =
      limeTable->GetNumberArray("botpose_orb_wpiblue", std::span<double>());

  if (mt2Array.size() == 0)
    return frc::Pose3d();

  megaTag2Pose =
      frc::Pose3d(units::meter_t{mt2Array[0]}, units::meter_t{mt2Array[1]},
                  units::meter_t{mt2Array[2]},
                  frc::Rotation3d(units::degree_t{mt2Array[3]},
...
frc::Pose3d AprilTagsSensor::getTargetToBotPose() {
  ...
  std::vector<double> targetToBot =
      limeTable->GetNumberArray("targetpose_robotspace", std::span<double>());

  if (targetToBot.size() == 0)
    return frc::Pose3d();

  return frc::Pose3d(units::meter_t{targetToBot[0]},
                     units::meter_t{targetToBot[1]},
                     units::meter_t{targetToBot[2]},
                     frc::Rotation3d(units::degree_t{targetToBot[3]},
...
frc::Pose3d AprilTagsSensor::get_botpose_targetspace() {
  ...
  std::vector<double> targetToBot =
      limeTable->GetNumberArray("botpose_targetspace", std::span<double>());
...
void AprilTagsSensor::InitSendable(wpi::SendableBuilder &builder) {
  ...
  builder.AddDoubleArrayProperty(
      ...
      [this] {
        ...
        botPose.push_back(
            currState.ToPose2d().Rotation().Degrees().to<double>());
        return botPose;
      },
      nullptr);
  builder.AddDoubleArrayProperty(
      ...
      [this] {
        ...
        botPose.push_back(
            megaTag2Pose.ToPose2d().Rotation().Degrees().to<double>());
        return botPose;
      },
      nullptr);
  builder.AddDoubleArrayProperty(
      ...
      [this] {
        return limeTable->GetNumberArray(
            ...
            std::array<double, 6>{
                cameraPose.X().to<double>(), cameraPose.Y().to<double>(),
  ...
  builder.AddDoubleProperty(
      "Vision acceptance outlier",
      [this] { return normalVisionOutlier.to<double>(); }, nullptr);
