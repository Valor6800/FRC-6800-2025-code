...
#define MAKE_VECTOR(angle)                                                     \
  Eigen::Vector2d {                                                            \
    units::math::cos(angle).value(), units::math::sin(angle).value()           \
  }
...
template class valor::Swerve<valor::PhoenixController<>,
                             valor::PhoenixController<>>;

template <class AzimuthMotor, class DriveMotor>
Swerve<AzimuthMotor, DriveMotor>::Swerve(
    frc::TimedRobot *_robot, const char *_name,
    std::vector<std::pair<AzimuthMotor *, DriveMotor *>> modules,
    units::meter_t _module_radius, units::meter_t _wheelDiameter)
    : valor::BaseSubsystem(_robot, _name), lockingToTarget(false),
      useCarpetGrain(false) {
  frc2::CommandScheduler::GetInstance().RegisterSubsystem(this);

  wpi::SendableRegistry::Add(&rot_controller, "rot_controller");
  ...
  wpi::array<frc::Translation2d, MODULE_COUNT> motorLocations =
      wpi::array<frc::Translation2d, MODULE_COUNT>(wpi::empty_array);
  for (size_t i = 0; i < MODULE_COUNT; i++) {
    motorLocations[i] =
        frc::Translation2d{_module_radius * MDX[i], _module_radius * MDY[i]};
    swerveModules.push_back(new valor::SwerveModule<AzimuthMotor, DriveMotor>(
        modules[i].first, modules[i].second, motorLocations[i],
        _wheelDiameter));
  }
  ...
  maxRotationSpeed = units::radians_per_second_t{
      (swerveModules[0]->getMaxDriveSpeed() / _module_radius).value()};

  kinematics = std::make_unique<frc::SwerveDriveKinematics<MODULE_COUNT>>(
      motorLocations);
  rawEstimator = std::make_unique<frc::SwerveDrivePoseEstimator<MODULE_COUNT>>(
      *kinematics, getGyro(), getModuleStates(), frc::Pose2d{0_m, 0_m, 0_rad});
  calcEstimator = std::make_unique<frc::SwerveDrivePoseEstimator<MODULE_COUNT>>(
      *kinematics, getGyro(), getModuleStates(), frc::Pose2d{0_m, 0_m, 0_rad});
  alignEstimator =
      std::make_unique<frc::SwerveDrivePoseEstimator<MODULE_COUNT>>(
          *kinematics, getGyro(), getModuleStates(),
          frc::Pose2d{0.001_m, 0.001_m, 0.001_rad});

  rawPosePublisher =
      nt::NetworkTableInstance::GetDefault()
          .GetStructTopic<frc::Pose2d>(
              "LiveWindow/BaseSubsystem/SwerveDrive/Actual Raw Pose")
          .Publish();
  calculatedPosePublisher =
      nt::NetworkTableInstance::GetDefault()
          .GetStructTopic<frc::Pose2d>(
              "LiveWindow/BaseSubsystem/SwerveDrive/Actual Calculated Pose")
          .Publish();
  alignPosePublisher =
      nt::NetworkTableInstance::GetDefault()
          .GetStructTopic<frc::Pose2d>(
              "LiveWindow/BaseSubsystem/SwerveDrive/Align Pose")
          .Publish();
  robotVelocitiesPublisher =
      nt::NetworkTableInstance::GetDefault()
          .GetStructTopic<frc::ChassisSpeeds>(
              "LiveWindow/BaseSubsystem/SwerveDrive/Robot Velocities")
          .Publish();
  ...
  getAllModuleStatesAsVector(); previousSetpoint = SwerveSetpoint(currentSpeeds,
  currentStates, DriveFeedforwards::zeros(config.numModules));*/
...
void Swerve<AzimuthMotor, DriveMotor>::init() {
  ...
  rot_controller.EnableContinuousInput(units::radian_t{-M_PI},
                                       units::radian_t{M_PI});
  for (size_t i = 0; i < MODULE_COUNT; i++) {
...
void Swerve<AzimuthMotor, DriveMotor>::assessInputs() {
  ...
  rotSpeed = std::copysign(
      std::fmin(std::fabs(rotationLerping(driverGamepad->rightStickX(1))),
                gcem::inv_sqrt(2)),
      driverGamepad->rightStickX(1));
...
void Swerve<AzimuthMotor, DriveMotor>::analyzeDashboard() {
  ...
  rawEstimator->UpdateWithTime(frc::Timer::GetFPGATimestamp(), getGyro(),
                               getModuleStates());
  calcEstimator->UpdateWithTime(frc::Timer::GetFPGATimestamp(), getGyro(),
                                getModuleStates());
  alignEstimator->UpdateWithTime(frc::Timer::GetFPGATimestamp(), getGyro(),
                                 getModuleStates());
  ...
  if (rotAlign) {
    rotSpeedRPS =
        ...
        rot_controller.GetSetpoint().velocity;
  }

  units::meters_per_second_t moduleSpeedsRotation = units::math::abs(
      units::meters_per_second_t{rotSpeedRPS.to<double>() *
                                 Constants::driveBaseRadius().to<double>()});
  units::meters_per_second_t moduleSpeedsTranslation =
      units::meters_per_second_t{sqrtf(powf(xSpeedMPS.to<double>(), 2) +
                                       powf(ySpeedMPS.to<double>(), 2))};

  if (moduleSpeedsTranslation + moduleSpeedsRotation > maxDriveSpeed) {
    units::meters_per_second_t adjustedModuleSpeedsTranslation =
        std::max(maxDriveSpeed - moduleSpeedsRotation, 0_mps);
  ...
  calculated_x_controller_val = x_controller.Calculate(
      xDistance,
      {xGoalAlign, 0.0_mps}); // endState not going to min controller velocity
  if (yAlign) {
    ...
    relativeToTagSpeed = (units::meters_per_second_t)(
        -yAlignPID.P *
            (yDistance - y_controller.GetSetpoint().position).value() -
        yAlignPID.D *
            (yTagRelativeVelocity - y_controller.GetSetpoint().velocity)
                .value() +
        yAlignPID.aFF * y_controller.GetSetpoint().velocity.value());
    relativeToTagXSpeed = (units::meters_per_second_t)(
        -xAlignPID.P *
            (xDistance - x_controller.GetSetpoint().position).value() -
        xAlignPID.D *
            (xTagRelativeVelocity - x_controller.GetSetpoint().velocity)
                .value() +
        xAlignPID.aFF * x_controller.GetSetpoint().velocity.value());

    if (units::math::fabs(yGoalAlign - yDistance) <= 1_in) {
      relativeToTagXSpeed =
          units::math::min(relativeToTagXSpeed, X_CONTROLLER_MIN_SPEED);
    }

    yAlignVector =
        MAKE_VECTOR(targetAngle - 90_deg) * relativeToTagSpeed.value();
    if (dumbAutoAlign) {
    ...
    // powerVector *= dotProduct / std::fabs(dotProduct);
...
bool Swerve<AzimuthMotor, DriveMotor>::yControllerAligned() {
  return units::math::abs((units::meter_t)yDistance - yGoalAlign) <
         yPosTolerance;
}
...
template <class AzimuthMotor, class DriveMotor>
void Swerve<AzimuthMotor, DriveMotor>::setRotAlignOffset(
...
void Swerve<AzimuthMotor, DriveMotor>::transformControllerSpeeds() {
  ...
  Eigen::Vector2d currVelocitiesFieldSpace{fieldSpaceSpeeds.vx.value(),
                                           fieldSpaceSpeeds.vy.value()};

  yTagRelativeVelocity = units::meters_per_second_t{
      currVelocitiesFieldSpace.dot(MAKE_VECTOR(targetAngle - 90_deg))};
  xTagRelativeVelocity = units::meters_per_second_t{
      currVelocitiesFieldSpace.dot(MAKE_VECTOR(targetAngle))};
}

template <class AzimuthMotor, class DriveMotor>
void Swerve<AzimuthMotor, DriveMotor>::driveRobotRelative(
...
void Swerve<AzimuthMotor, DriveMotor>::updateAngularAcceleration() {
  ...
  units::angular_velocity::radians_per_second_t currentYawRate =
...
Swerve<AzimuthMotor, DriveMotor>::getSmoothedAngularAcceleration() {
  if (yawRateBuffer.empty())
    return 0_rad_per_s_sq;

  units::angular_acceleration::radians_per_second_squared_t sum =
      0_rad_per_s_sq;
...
frc::ChassisSpeeds Swerve<AzimuthMotor, DriveMotor>::getRobotRelativeSpeeds() {
  wpi::array<frc::SwerveModuleState, 4> moduleStates = {
      swerveModules[0]->getState(), swerveModules[1]->getState(),
...
frc::ChassisSpeeds Swerve<AzimuthMotor, DriveMotor>::getFieldRelativeSpeeds() {
  return frc::ChassisSpeeds::FromRobotRelativeSpeeds(
      ...
      calcEstimator->GetEstimatedPosition().Rotation());
}
...
void Swerve<AzimuthMotor, DriveMotor>::updateAngularPosition() {
  ...
  angularPosition = units::math::fmod(angularPosition + units::radian_t(M_PI),
                                      units::radian_t{2.0 * M_PI});
  ...
  calcEstimator->AddVisionMeasurement(
      frc::Pose2d(0_m, 0_m, angularPosition), 0_s,
      {std::numeric_limits<double>::max(), std::numeric_limits<double>::max(),
       1.0});
}

template <class AzimuthMotor, class DriveMotor>
void Swerve<AzimuthMotor, DriveMotor>::setSwerveDesiredState(
    wpi::array<frc::SwerveModuleState, MODULE_COUNT> desiredStates,
...
template <class AzimuthMotor, class DriveMotor>
void Swerve<AzimuthMotor, DriveMotor>::enableCarpetGrain(
    double _grainMultiplier, bool _roughTowardsRed) {
...
template <class AzimuthMotor, class DriveMotor>
void Swerve<AzimuthMotor, DriveMotor>::setXConstraints(
    frc::TrapezoidProfile<units::meter>::Constraints constraints) {
  x_controller.SetConstraints(constraints);
}

template <class AzimuthMotor, class DriveMotor>
frc::TrapezoidProfile<units::meter>::Constraints
Swerve<AzimuthMotor, DriveMotor>::getXConstraints() {
...
void Swerve<AzimuthMotor, DriveMotor>::calculateCarpetPose() {
  ...
  calcEstimator->AddVisionMeasurement(
      frc::Pose2d{factor * deltaX + previousPose.X(), newPose.Y(),
                  newPose.Rotation()},
...
template <class AzimuthMotor, class DriveMotor>
void Swerve<AzimuthMotor, DriveMotor>::setupGyro(int _pigeonCanID,
                                                 const char *_pigeonCanBus,
                                                 units::degree_t mountRoll,
                                                 units::degree_t mountPitch,
                                                 units::degree_t mountYaw) {
  pigeon.reset();
  pigeon = std::make_unique<ctre::phoenix6::hardware::Pigeon2>(_pigeonCanID,
                                                               _pigeonCanBus);
  pigeon->GetConfigurator().Apply(
      ctre::phoenix6::configs::Pigeon2Configuration{}.WithMountPose(
...
void Swerve<AzimuthMotor, DriveMotor>::resetGyro() {
  ...
  frc::Pose2d desiredPose =
      frc::Pose2d(initialPose.X(), initialPose.Y(), frc::Rotation2d(0_deg));
...
template <class AzimuthMotor, class DriveMotor>
wpi::array<frc::SwerveModulePosition, MODULE_COUNT>
Swerve<AzimuthMotor, DriveMotor>::getModuleStates() {
  wpi::array<frc::SwerveModulePosition, MODULE_COUNT> modulePositions =
      wpi::array<frc::SwerveModulePosition, MODULE_COUNT>(wpi::empty_array);
  for (size_t i = 0; i < swerveModules.size(); i++) {
...
template <class AzimuthMotor, class DriveMotor>
wpi::array<frc::SwerveModuleState, MODULE_COUNT>
Swerve<AzimuthMotor, DriveMotor>::getModuleStates(
    units::velocity::meters_per_second_t vx_mps,
    units::velocity::meters_per_second_t vy_mps,
    units::angular_velocity::radians_per_second_t omega_radps, bool isFOC) {
  frc::ChassisSpeeds chassisSpeeds =
      isFOC ? frc::ChassisSpeeds::FromFieldRelativeSpeeds(
...
template <class AzimuthMotor, class DriveMotor>
wpi::array<frc::SwerveModuleState, MODULE_COUNT>
Swerve<AzimuthMotor, DriveMotor>::getAllModuleStates() {
  wpi::array<frc::SwerveModuleState, MODULE_COUNT> moduleStates =
      wpi::array<frc::SwerveModuleState, MODULE_COUNT>(wpi::empty_array);
  for (size_t i = 0; i < swerveModules.size(); i++) {
...
valor::Swerve<AzimuthMotor, DriveMotor>::getAllModuleStatesAsVector() {
...
double Swerve<AzimuthMotor, DriveMotor>::getSkiddingRatio() {
  wpi::array<frc::SwerveModuleState, MODULE_COUNT> measuredModuleStates =
      getAllModuleStates();
  double angularVelocityMeasured =
      kinematics->ToChassisSpeeds(measuredModuleStates).omega.value();
  wpi::array<frc::SwerveModuleState, MODULE_COUNT> swerveStatesRotPart =
      kinematics->ToSwerveModuleStates(frc::ChassisSpeeds{
          0_mps, 0_mps, units::degrees_per_second_t{angularVelocityMeasured}});
  wpi::array<double, MODULE_COUNT> swerveStatesTranslationalPartMag =
      wpi::array<double, MODULE_COUNT>(wpi::empty_array);

  for (int i = 0; i < MODULE_COUNT; i++) {
    frc::Translation2d swerveStateAsVector =
                           swerveModules[i]->convertSwerveStateToVelocityVector(
                               measuredModuleStates[i]),
                       swerveStatesRotPartAsVector =
                           swerveModules[i]->convertSwerveStateToVelocityVector(
                               swerveStatesRotPart[i]),
                       swerveStatesTranslationalPartAsVector =
                           swerveStateAsVector - swerveStatesRotPartAsVector;
    swerveStatesTranslationalPartMag[i] =
        swerveStatesTranslationalPartAsVector.Norm().to<double>();
  }
  double maxTransSpeed = 0.0;
  double minTransSpeed = 10000.0;
  for (size_t i = 0; i < swerveStatesTranslationalPartMag.size(); i++) {
    maxTransSpeed =
        std::max(maxTransSpeed, swerveStatesTranslationalPartMag[i]);
    minTransSpeed =
        std::min(minTransSpeed, swerveStatesTranslationalPartMag[i]);
  }
  if (minTransSpeed < 0.05)
    minTransSpeed = 0;
  if (maxTransSpeed < 0.3)
    return maxTransSpeed - minTransSpeed;
  if (minTransSpeed == 0)
    return maxTransSpeed;
  return maxTransSpeed / minTransSpeed;
}
...
double Swerve<AzimuthMotor, DriveMotor>::rotationLerping(double input) {
  ...
  if (abs > P4.first) {
    rot = std::copysign(
        (((P5.second - P4.second) / (P5.first - P4.first)) * (abs - P4.first)) +
            P4.second,
        input);
  } else if (abs > P3.first) {
    rot = std::copysign(
        (((P4.second - P3.second) / (P4.first - P3.first)) * (abs - P3.first)) +
            P3.second,
        input);
  } else if (abs > P2.first) {
    rot = std::copysign(
        (((P3.second - P2.second) / (P3.first - P2.first)) * (abs - P2.first)) +
            P2.second,
        input);
  } else {
    rot = std::copysign(
        (((P2.second - P1.second) / (P2.first - P1.first)) * (abs - P1.first)) +
            P1.second,
        input);
  }
  return rot;
...
template <class AzimuthMotor, class DriveMotor>
void Swerve<AzimuthMotor, DriveMotor>::InitSendable(
    wpi::SendableBuilder &builder) {
  builder.SetSmartDashboardType("Subsystem");
  builder.AddDoubleProperty(
      "Commanded X Speed", [this] { return xSpeed; }, nullptr);
  builder.AddDoubleProperty(
      "Commanded Y Speed", [this] { return ySpeed; }, nullptr);
  builder.AddDoubleProperty(
      "Commanded Rot Speed", [this] { return rotSpeed; }, nullptr);
  builder.AddDoubleProperty(
      "Commanded X Speed MPS", [this] { return xSpeedMPS.value(); }, nullptr);
  builder.AddDoubleProperty(
      "Commanded Y Speed MPS", [this] { return ySpeedMPS.value(); }, nullptr);
  builder.AddDoubleProperty(
      "Commanded Rot Speed RPS", [this] { return rotSpeedRPS.value(); },
      nullptr);
  builder.AddDoubleProperty(
      "Gyro Pitch", [this] { return pigeon->GetPitch().GetValueAsDouble(); },
      nullptr);
  builder.AddDoubleProperty(
      "Gyro Yaw", [this] { return pigeon->GetYaw().GetValueAsDouble(); },
      nullptr);
  builder.AddDoubleProperty(
      "Gyro Roll", [this] { return pigeon->GetRoll().GetValueAsDouble(); },
      nullptr);
  ...
  builder.AddDoubleArrayProperty(
      ...
      [this] {
        ...
        states.push_back(
            swerveModules[0]->getState().angle.Degrees().template to<double>());
        states.push_back(
            swerveModules[0]->getState().speed.template to<double>());
        states.push_back(
            swerveModules[1]->getState().angle.Degrees().template to<double>());
        states.push_back(
            swerveModules[1]->getState().speed.template to<double>());
        states.push_back(
            swerveModules[2]->getState().angle.Degrees().template to<double>());
        states.push_back(
            swerveModules[2]->getState().speed.template to<double>());
        states.push_back(
            swerveModules[3]->getState().angle.Degrees().template to<double>());
        states.push_back(
            swerveModules[3]->getState().speed.template to<double>());
        return states;
      },
      nullptr);
  ...
  builder.AddDoubleArrayProperty(
      ...
      [this] {
        return std::vector<double>{joystickVector[0], joystickVector[1]};
      },
      nullptr);
  builder.AddDoubleArrayProperty(
  ...
  builder.AddDoubleProperty(
      "Align: Target Angle",
      [this] { return units::radian_t{targetAngle}.to<double>(); }, nullptr);

  builder.AddDoubleProperty(
      "Align: Y Distance", [this] { return yDistance.to<double>(); }, nullptr);
  builder.AddDoubleProperty(
      "Align: X Distance", [this] { return xDistance.to<double>(); }, nullptr);

  builder.AddBooleanProperty(
      "Align: Locking on Target", [this] { return yAlign && rotAlign; },
      nullptr);
  builder.AddBooleanProperty(
      "Align: Rotation Align", [this] { return rotAlign; }, nullptr);
  builder.AddBooleanProperty(
      "Align: Y Align", [this] { return yAlign; }, nullptr);
  builder.AddBooleanProperty(
      "Align: X Align", [this] { return xAlign; }, nullptr);

  builder.AddDoubleProperty(
      "Align: Y Goal Target", [this] { return yGoalAlign.value(); }, nullptr);

  builder.AddDoubleProperty(
      "Align: X Goal Target", [this] { return xGoalAlign.value(); }, nullptr);

  builder.AddDoubleProperty(
      "Align: Y Tag Relative Velocity",
      [this] { return yTagRelativeVelocity.value(); }, nullptr);

  builder.AddDoubleProperty(
      "Align: X Tag Relative Velocity",
      [this] { return xTagRelativeVelocity.value(); }, nullptr);
  ...
  builder.AddDoubleProperty(
      ...
      [this] {
        return static_cast<double>(rot_controller.GetSetpoint().position());
      },
      nullptr);
  builder.AddDoubleProperty(
      "Align Controller: Y Controller: Y Controller Setpoint",
      [this] {
        return static_cast<double>(y_controller.GetSetpoint().position());
      },
      nullptr);
  builder.AddDoubleProperty(
      "Align Controller: Y Controller: Y Controller Setpoint Velocity",
      [this] {
        return static_cast<double>(y_controller.GetSetpoint().velocity());
      },
      nullptr);
  builder.AddDoubleProperty(
      "Align Controller: Y Controller: Y Controller P Value",
      [this] { return yAlignPID.P; }, nullptr);
  builder.AddDoubleProperty(
      "Align Controller: X Controller: X Controller Setpoint",
      [this] {
        return static_cast<double>(x_controller.GetSetpoint().position());
      },
      nullptr);
  builder.AddDoubleProperty(
      "Align Controller: X Controller: X Controller Setpoint Velocity",
      [this] {
        return static_cast<double>(x_controller.GetSetpoint().velocity());
      },
      nullptr);
  builder.AddDoubleProperty(
      "Align Controller: Calculated y_controller value",
      [this] { return calculated_y_controller_val; }, nullptr);
  builder.AddDoubleProperty(
      "Align Controller: Calculated x_controller value",
      [this] { return calculated_x_controller_val; }, nullptr);
  builder.AddDoubleProperty(
      "Align Controller: Rot Controller: Rot Controller Position Error",
      [this] { return rot_controller.GetPositionError().value(); }, nullptr);
  builder.AddDoubleProperty(
      "Align Controller: Y Controller: Y Controller Position Error",
      [this] { return y_controller.GetPositionError().value(); }, nullptr);
  builder.AddDoubleProperty(
      "Align Controller: X Controller: X Controller Position Error",
      [this] { return x_controller.GetPositionError().value(); }, nullptr);
  builder.AddDoubleProperty(
      "Align Controller: Rot Controller: Rot Controller Velocity Error",
      [this] { return rot_controller.GetVelocityError().value(); }, nullptr);
  builder.AddDoubleProperty(
      "Align Controller: Y Controller: Y Controller Velocity Error",
      [this] { return y_controller.GetVelocityError().value(); }, nullptr);
  builder.AddDoubleProperty(
      "Align Controller: X Controller: X Controller Velocity Error",
      [this] { return x_controller.GetVelocityError().value(); }, nullptr);
  ...
  builder.AddDoubleProperty(
      "Skidding Ratio", [this] { return getSkiddingRatio(); }, nullptr);

  builder.AddBooleanProperty(
      "Is Robot Skidding", [this] { return isRobotSkidding(); }, nullptr);
