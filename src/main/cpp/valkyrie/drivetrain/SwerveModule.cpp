...
template class valor::SwerveModule<valor::PhoenixController<>,
                                   valor::PhoenixController<>>;

template <class AzimuthMotor, class DriveMotor>
SwerveModule<AzimuthMotor, DriveMotor>::SwerveModule(
    AzimuthMotor *_azimuthMotor, DriveMotor *_driveMotor,
    frc::Translation2d _wheelLocation, units::meter_t wheelDiameter)
    : azimuthMotor(_azimuthMotor), driveMotor(_driveMotor),
      wheelConversion(M_PI * wheelDiameter / 1_tr) {
  if (_wheelLocation.X() > 0_m && _wheelLocation.Y() > 0_m)
    wheelIdx = 0;
  else if (_wheelLocation.X() > 0_m && _wheelLocation.Y() < 0_m)
    wheelIdx = 1;
  else if (_wheelLocation.X() < 0_m && _wheelLocation.Y() > 0_m)
    wheelIdx = 3;
  else
...
SwerveModule<AzimuthMotor, DriveMotor>::getDriveSpeed() {
  return driveMotor->getSpeed() * wheelConversion;
}

template <class AzimuthMotor, class DriveMotor>
units::meters_per_second_t
SwerveModule<AzimuthMotor, DriveMotor>::getMaxDriveSpeed() {
  return driveMotor->getMaxMechSpeed() * wheelConversion;
}

template <class AzimuthMotor, class DriveMotor>
frc::SwerveModulePosition
SwerveModule<AzimuthMotor, DriveMotor>::getModulePosition() {
...
template <class AzimuthMotor, class DriveMotor>
void SwerveModule<AzimuthMotor, DriveMotor>::setDesiredState(
    frc::SwerveModuleState _desiredState, bool isDriveOpenLoop) {
  // Deadband
...
template <class AzimuthMotor, class DriveMotor>
bool SwerveModule<AzimuthMotor, DriveMotor>::loadAndSetAzimuthZeroReference(
    std::vector<units::turn_t> offsets) {
  // Read the encoder position. If the encoder position isn't returned, set the
  // position to what the wheels
  //   are currently. The pit crew sets the wheels straight in pre-match setup.
  //   They should be close enough if the mag encoders aren't working. Protects
  //   against issues as seen in: https://www.youtube.com/watch?v=MGxpWNcv-VM
...
// The angle coming in is an optimized angle. No further calcs should be done on
// 'angle'
template <class AzimuthMotor, class DriveMotor>
void SwerveModule<AzimuthMotor, DriveMotor>::setAzimuthPosition(
    frc::Rotation2d desiredAngle) {
  azimuthMotor->setPosition(
      units::turn_t{desiredAngle.Radians().value() / (2.0 * M_PI)});
}

template <class AzimuthMotor, class DriveMotor>
void SwerveModule<AzimuthMotor, DriveMotor>::setAzimuthPower(
    units::volt_t voltage) {
  azimuthMotor->setPower(voltage);
}

template <class AzimuthMotor, class DriveMotor>
void SwerveModule<AzimuthMotor, DriveMotor>::setDrivePower(
    units::volt_t voltage) {
  driveMotor->setPower(voltage);
}

template <class AzimuthMotor, class DriveMotor>
void SwerveModule<AzimuthMotor, DriveMotor>::setDriveOpenLoop(
    units::meters_per_second_t mps) {
  driveMotor->setPower(mps / getMaxDriveSpeed() * units::volt_t{12});
}

template <class AzimuthMotor, class DriveMotor>
void SwerveModule<AzimuthMotor, DriveMotor>::setDriveClosedLoop(
    units::meters_per_second_t speed) {
  auto outSpeed =
      units::turns_per_second_t{speed.value() / wheelConversion.value()};
  driveMotor->setSpeed(outSpeed);
}

template <class AzimuthMotor, class DriveMotor>
void SwerveModule<AzimuthMotor, DriveMotor>::setUpdateFrequency(
    units::frequency::hertz_t freq) {
...
template <class AzimuthMotor, class DriveMotor>
frc::Translation2d
SwerveModule<AzimuthMotor, DriveMotor>::convertSwerveStateToVelocityVector(
    frc::SwerveModuleState state) {
  return frc::Translation2d{units::meter_t{state.speed.to<double>()},
                            state.angle};
}

template <class AzimuthMotor, class DriveMotor>
void SwerveModule<AzimuthMotor, DriveMotor>::InitSendable(
  ...
  builder.AddDoubleProperty(
      ...
      nullptr);
  builder.AddDoubleProperty(
      "state: speed", [this] { return getState().speed.template to<double>(); },
      nullptr);
  builder.AddDoubleProperty(
      "desired state: angle",
      ...
      nullptr);
  builder.AddDoubleProperty(
      "desired state: speed", [this] { return desiredState.speed.value(); },
      nullptr);
  builder.AddDoubleProperty(
      "state: distance",
