...
void Auto::preloadSelectedAuto() {
  if (m_chooser.GetSelected() != "" &&
      m_chooser.GetSelected().find("NONE") == std::string::npos) {
    preloadAuto(m_chooser.GetSelected());
  }
}

void Auto::clearAutos() { loadedAutos.clear(); }
...
bool is_caps(char c) { return (c >= 'A' && c <= 'Z'); }

std::string makeFriendlyName(std::string filename) {
  // take last part of the path string when divided with /'s - this should be
  // the filename
  ...
  for (uint i = 0; i < filename.length(); i++) {
    ...
    if (filename[i] == '.') {
      break;
               ...
               '_') { // replace _'s with spaces for a snake case filename
      // make sure we dont have double spaces
      if (*(n_name.end() - 1) != ' ')
        n_name += ' ';
    } else if (i >= 1 && is_alpha(filename[i]) && is_caps(filename[i]) &&
               !is_caps(filename[i - 1]) &&
               *(n_name.end() - 1) !=
                   ' ') { // check for camel case, add space if present
      n_name += ' ';
...
std::vector<std::string> listDirectory(std::string path_name) {
  ...
  for (const auto &entry : std::filesystem::directory_iterator(path_name)) {
    if (entry.path().string().find("NONE") != std::string::npos)
      continue;
    std::string path = entry.path();
    if (path.find(".path") != std::string::npos ||
        path.find(".auto") != std::string::npos) {
...
void Auto::fillAutoList() {
  for (std::string path : listDirectory(AUTOS_PATH)) {
    ...
    m_chooser.AddOption(makeFriendlyName(removeFileType(path)),
                        removeFileType(path));
  }
