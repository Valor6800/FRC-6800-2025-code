...
Robot::Robot()
    : leds{this, LEDConstants::LED_COUNT, LEDConstants::LED_SEGMENTS,
           CANIDs::CANDLE},
      drivetrain(this, &leds), scorer(this, &drivetrain, &climber, &leds),
      climber(this, &leds), valorAuto() {
  frc::TimedRobot();
  leds.setColor(0, valor::CANdleSensor::OFF,
                valor::CANdleSensor::PRIORITY_DEFAULT);
...
void Robot::RobotInit() {
  frc::SmartDashboard::PutString("Robot Serial Number",
                                 Constants::serialNumber);
  // We could use frc2::cmd::None() instead of nullptr but it means that we have
  // to hardcode the index of the command in PIT_SEQUENCES or store it elsewhere
  // otherwise Command* will be dangling Probably easier to use nullptr and
...
void Robot::RobotPeriodic() {
  frc::SmartDashboard::PutNumber(
      "Battery Voltage", frc::RobotController::GetBatteryVoltage().value());
...
void Robot::DisabledPeriodic() { valorAuto.preloadSelectedAuto(); }
...
void Robot::AutonomousExit() { autoCommands.Cancel(); }
...
void Robot::TestInit() {
  ...
  pitSequenceCommand =
      frc2::cmd::Sequence(frc2::cmd::RunOnce([this] {
                            for (int i = 8; i < LEDConstants::LED_COUNT; i++)
                              leds.setLED(i, valor::CANdleSensor::OFF);
                          }),
                          drivetrain.pitSequence(), subsystemGateCommand(),
...
void Robot::TestExit() { pitSequenceCommand.Cancel(); }
