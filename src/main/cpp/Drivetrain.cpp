...
#define ROT_CONTROLLER_SPEED_LIMIT                                             \
  units::degrees_per_second_t { 9 }
...
Drivetrain::Drivetrain(frc::TimedRobot *_robot, valor::CANdleSensor *_leds)
    : valor::Swerve<SwerveAzimuthMotor, SwerveDriveMotor>(
          _robot, "SwerveDrive", generateModules(), Constants::moduleDiff(),
          WHEEL_DIAMETER),
      teleopStart(999999999999),
      leftDistanceSensor(_robot, "LEFT Front Distance Sensor",
                         CANIDs::LEFT_CAN_RANGE_DRIVETRAIN_SENSOR, "",
                         VIABLE_ELEVATOR_DISTANCE - 1_mm),
      rightdistanceSensor(_robot, "RIGHT FRONT Distance Sensor",
                          CANIDs::RIGHT_CAN_RANGE_DRIVETRAIN_SENSOR, "",
                          VIABLE_ELEVATOR_DISTANCE - 1_mm),
      leds(_leds) {
  xPIDF.P = KPX;
  xPIDF.I = KIX;
  ...
  for (std::pair<const char *, frc::Pose3d> aprilCam :
       Constants::getAprilCameras()) {
    aprilTagSensors.push_back(
        new valor::AprilTagsSensor(robot, aprilCam.first, aprilCam.second));
    aprilTagSensors.back()->setPipe(valor::VisionSensor::PIPELINE_0);
  }

  state.dir = NONE;

  setupGyro(CANIDs::PIGEON_CAN, PIGEON_CAN_BUS, Constants::pigeonMountRoll(),
            Constants::pigeonMountPitch(), Constants::pigeonMountYaw());
  ...
  AutoBuilder::configure(
      ...
      [this](frc::ChassisSpeeds speeds, auto _) {
        driveRobotRelative(speeds);
      }, // Method that will drive the robot given ROBOT RELATIVE ChassisSpeeds
      std::shared_ptr<PPHolonomicDriveController>(
          new PPHolonomicDriveController(
              PIDConstants(xPIDF.P, xPIDF.I,
                           xPIDF.D), // Translation PID constants
              PIDConstants(thetaPIDF.P, thetaPIDF.I,
                           thetaPIDF.D) // Rotation PID constants
              )),
  ...
  poseErrorPPTopic =
      nt::NetworkTableInstance::GetDefault()
          .GetStructTopic<frc::Transform2d>(
              "LiveWindow/BaseSubsystem/SwerveDrive/Pose Error PP")
          .Publish();
  table->PutNumber("Y Controller Activation Degree Threshold",
                   Y_ACTIVATION_THRESHOLD.value());
  reefPublisher = nt::NetworkTableInstance::GetDefault()
                      .GetStructTopic<frc::Pose2d>("Reef Pose")
                      .Publish();
  robotPredictedPublisher =
      nt::NetworkTableInstance::GetDefault()
          .GetStructTopic<frc::Pose2d>("Robot Predicted Pose")
          .Publish();
  doubtRot = 1;
  robotPublisher = nt::NetworkTableInstance::GetDefault()
                       .GetStructTopic<frc::Pose2d>("Robot Pose")
                       .Publish();
  robotInTagSpacePublisher =
      nt::NetworkTableInstance::GetDefault()
          .GetStructTopic<frc::Transform2d>("Robot In Tag Space Transform")
          .Publish();
...
std::vector<std::pair<SwerveAzimuthMotor *, SwerveDriveMotor *>>
Drivetrain::generateModules() {
  ...
  for (size_t i = 0; i < 4; i++) {
    SwerveAzimuthMotor *azimuthMotor = new SwerveAzimuthMotor(
        Constants::getAzimuthMotorType(), CANIDs::AZIMUTH_CANS[i],
        valor::NeutralMode::Brake, Constants::swerveAzimuthsReversals()[i],
        PIGEON_CAN_BUS
    ...
    azimuthMotor->setupCANCoder(CANIDs::CANCODER_CANS[i],
                                Constants::swerveZeros()[i], false,
                                PIGEON_CAN_BUS);
    azimuthMotor->setContinuousWrap(true);
    azimuthMotor->applyConfig();

    SwerveAzimuthMotor *driveMotor = new SwerveDriveMotor(
        valor::PhoenixControllerType::KRAKEN_X60_FOC, CANIDs::DRIVE_CANS[i],
        valor::NeutralMode::Coast, Constants::swerveDrivesReversals()[i],
        PIGEON_CAN_BUS);
...
void Drivetrain::init() {
  ...
  currentPosePathPlanner =
      nt::NetworkTableInstance::GetDefault()
          .GetStructTopic<frc::Pose2d>("/PathPlanner/currentPose")
          .Subscribe(frc::Pose2d{});
  targetPosePathPlanner =
      nt::NetworkTableInstance::GetDefault()
          .GetStructTopic<frc::Pose2d>("/PathPlanner/targetPose")
          .Subscribe(frc::Pose2d{});
}

void Drivetrain::assessInputs() {
  if (!driverGamepad || !driverGamepad->IsConnected() || !operatorGamepad ||
      !operatorGamepad->IsConnected())
    return;
...
void Drivetrain::analyzeDashboard() {
  ...
  frc::Pose2d predictedPose =
      getCalculatedPose().Exp(getRobotRelativeSpeeds().ToTwist2d(.25_s));
  robotPredictedPublisher.Set(predictedPose);

  frc::Pose2d robotToCenter{
      predictedPose.Translation() +
          frc::Translation2d(-FIELD_LENGTH / 2.0, -FIELD_WIDTH / 2.0),
      predictedPose.Rotation()};

  std::pair<int, frc::Pose3d> temp = valor::getNearestTag(
      robotToCenter,
      frc::DriverStation::kRed == frc::DriverStation::GetAlliance()
          ? valor::redReefAprilTagPoses
          : valor::blueReefAprilTagPoses);

  frc::Pose2d reefTagPose{
      temp.second.ToPose2d().Translation() +
          frc::Translation2d(FIELD_LENGTH / 2.0, FIELD_WIDTH / 2.0),
      temp.second.ToPose2d().Rotation()};

  if (!state.alignToTarget) {
    state.reefTag = {temp.first, reefTagPose};
  }
  ...
  if (state.climberAlign) {
    Swerve::targetAngle =
        frc::DriverStation::GetAlliance() == frc::DriverStation::kRed ? 90_deg
                                                                      : -90_deg;
    Swerve::yAlign = false;
    Swerve::xAlign = false;
    ...
    state.netAngle =
        frc::DriverStation::GetAlliance() == frc::DriverStation::kRed ? 180_deg
                                                                      : 0_deg;
    if (units::math::abs(
            (frc::Rotation2d(state.netAngle) -
             (calcEstimator.get()->GetEstimatedPosition().Rotation()))
                .Degrees()) < 60_deg) {
    ...
    Swerve::yAlign =
        hasYReset && state.elevState != Constants::Scorer::ELEVATOR_STATE::ONE;
    Swerve::rotAlign = true;
    Swerve::xAlign =
        state.worldAlign ||
        (Swerve::yAlign &&
         units::math::fabs(Swerve::yDistance - Swerve::yGoalAlign).value() <
             table->GetNumber("X SHUTOFF Y DISTANCE",
                              X_SHUTOFF_Y_DISTANCE.value()) &&
  ...
  if (detectedAprilCam != nullptr && reefSpace.X() < 1.5_m) {
    ...
    state.yEstimate =
        Y_FILTER_CONST * state.yEstimate +
        ((1 - Y_FILTER_CONST) *
         detectedAprilCam->get_botpose_targetspace().X().to<double>());
    state.xEstimate =
        -detectedAprilCam->get_botpose_targetspace().Z().to<double>();
  ...
  for (valor::AprilTagsSensor *aprilLime : aprilTagSensors) {
    if (aprilLime->hasTarget() &&
      ...
      aprilLime->applyVisionMeasurement(
          calcEstimator.get(), getRobotSpeeds(),
          pigeon->GetAngularVelocityZWorld().GetValue(), doubtX, doubtY,
          doubtRot);
    }
  ...
  if (frc::Timer::GetFPGATimestamp() - teleopStart >
          TELEOP_MAX_TIME - TIME_TO_RUMBLE &&
      frc::Timer::GetFPGATimestamp() - teleopStart <
          TELEOP_MAX_TIME - (TIME_TO_RUMBLE - 3_s)) {
...
void Drivetrain::assignOutputs() {
  ...
  state.accel = {pigeon->GetAccelerationX().GetValue(),
...
frc::Transform2d Drivetrain::reefTransformConversion(frc::Pose2d reefTagPose) {
  frc::Transform2d reefTagTransform{reefTagPose.Translation(),
                                    reefTagPose.Rotation()};

  frc::Transform2d robotTransform{getCalculatedPose().Translation(),
                                  getCalculatedPose().Rotation()};

  frc::Transform2d robotInTagSpaceTransform =
      reefTagTransform.Inverse() + robotTransform;
...
units::meters_per_second_t Drivetrain::getRobotSpeeds() {
  return units::meters_per_second_t{
      sqrtf(powf(getRobotRelativeSpeeds().vx.to<double>(), 2) +
            powf(getRobotRelativeSpeeds().vy.to<double>(), 2))};
}
...
frc2::FunctionalCommand *Drivetrain::getResetOdom() {
  return new frc2::FunctionalCommand(
      ...
      [&] { // continuously running
        ...
        for (valor::AprilTagsSensor *aprilLime : aprilTagSensors) {
          if (aprilLime->hasTarget() &&
              (aprilLime->getSensor().ToPose2d().X() > 0_m &&
               aprilLime->getSensor().ToPose2d().Y() > 0_m)) {
            table->PutNumber("resetting odom",
                             table->GetNumber("resetting odom", 0) + 1);
...
std::bitset<5> Drivetrain::getAutoDunkAcceptance() {
  ...
  acceptance[1] = withinXRange(
      frc::DriverStation::IsAutonomous()
          ? .34_m
          : (units::meter_t)table->GetNumber("Viable Dunk Distance (m)",
                                             VIABLE_DUNK_DISTANCE.value()));
  acceptance[2] =
      units::math::fabs(yTagRelativeVelocity) < Y_CONTROLLER_SPEED_LIMIT;
  acceptance[3] =
      units::math::fabs(getYawVelocity()) < ROT_CONTROLLER_SPEED_LIMIT;

  units::meters_per_second_t autoDunkSpeedThreshold =
      Constants::Scorer::getAutoDunkSpeedLimitation(xAlign);
  acceptance[4] = isSpeedBelowThreshold(frc::DriverStation::IsAutonomous()
                                            ? AUTO_VIABLE_DUNK_SPEED_L4
                                            : autoDunkSpeedThreshold);
  return acceptance;
}

bool Drivetrain::autoAlignShutOff() { return getAutoDunkAcceptance().all(); }

frc2::CommandPtr
Drivetrain::pitSequenceCommand(const frc::ChassisSpeeds &speeds, int offset,
                               int numLeds) {
  auto targetStates = getModuleStates(speeds);
  return frc2::cmd::Sequence(
      frc2::cmd::Deadline(frc2::cmd::Wait(3_s),
      ...
      frc2::cmd::RunOnce([this, offset, numLeds] {
        ...
        for (int i = 0; i < numLeds; i++) {
          ...
          leds->setLED(LEDConstants::LED_COUNT - offset - i,
...
frc2::CommandPtr Drivetrain::pitSequence() {
  return frc2::cmd::Sequence(
      pitSequenceCommand(
          frc::ChassisSpeeds{0_mps, maxDriveSpeed / 3.0, 0_rad_per_s}, 0, 2),
      pitSequenceCommand(
          frc::ChassisSpeeds(0_mps, 0_mps, maxRotationSpeed / 3.0), 2, 2),
      pitSequenceCommand(
          frc::ChassisSpeeds{maxDriveSpeed / 3.0, 0_mps, 0_rad_per_s}, 4, 2),
      pitSequenceCommand(
          frc::ChassisSpeeds{-maxDriveSpeed / 3.0, 0_mps, 0_rad_per_s}, 6, 2),
...
void Drivetrain::alignAngleZoning() {
  ...
  if (frc::DriverStation::GetAlliance() == frc::DriverStation::Alliance::kRed) {
    if (robotX >= 13_m) {
      if (robotY >= 4.5_m)
        Swerve::targetAngle = RED_REEF_8_ANGLE;
      if (robotY <= 3.5_m)
        Swerve::targetAngle = RED_REEF_6_ANGLE;
      else
        Swerve::targetAngle = RED_REEF_7_ANGLE;
    } else {
      if (robotY >= 4.5_m)
        Swerve::targetAngle = RED_REEF_9_ANGLE;
      if (robotY <= 3.5_m)
        Swerve::targetAngle = RED_REEF_11_ANGLE;
      else
        Swerve::targetAngle = RED_REEF_10_ANGLE;
    }
  }
  // blue alliance TODO
  else {
...
bool Drivetrain::withinYRange() { return yControllerAligned(); }
...
bool Drivetrain::isSpeedBelowThreshold(units::meters_per_second_t m) {
  ...
  units::meters_per_second_t totalSpeed =
      units::math::hypot(speeds.vx, speeds.vy);
  return (totalSpeed < m);
  // (units::meters_per_second_t) table->GetNumber("Viable Dunk speed",
  // VIABLE_DUNK_SPEED.value())
}
...
void Drivetrain::choosePoleDirection(Direction dir, Constants::AprilTag tag) {
  std::unordered_map<Constants::AprilTag, Constants::DirectionalOffSet>
      poleOffset = frc::DriverStation::GetAlliance() == frc::DriverStation::kRed
                       ...
                       : Constants::bluePoleOffsets;
  units::inch_t offset = poleOffset.find(tag) != poleOffset.end()
                             ? poleOffset.at(tag).at(dir)
                             : 0.0_in;
  switch (dir) {
  case LEFT:
  ...
  case RIGHT:
    ...
    break;
  default:
    Swerve::yGoalAlign = poleOffset.find(tag) != poleOffset.end()
                             ? poleOffset.at(tag).at(NONE)
                             : 0_in;
    break;
  }
}
  ...
  builder.AddBooleanProperty(
      ...
      [this] {
        return units::acceleration::meters_per_second_squared_t{
                   sqrtf(powf(state.accel.x.to<double>(), 2) +
                         powf(state.accel.y.to<double>(), 2))} >
               20.0_mps_sq; // ~60 kg bot -> 600 N, 5 measurements * 20ms = .1s,
                            // impulse = .1 * 600 = 60 Joules
      },
      nullptr);
  builder.AddDoubleProperty(
      "Locking Tag ID", [this] { return state.reefTag.first; }, nullptr);

  builder.AddDoubleProperty("Direction", [this] { return state.dir; }, nullptr);

  builder.AddDoubleProperty(
      "Lidar Distance", [this] { return lidarDistance().value(); }, nullptr);
  builder.AddBooleanProperty(
      "Within Y Range", [this] { return withinYRange(); }, nullptr);
  builder.AddDoubleProperty(
      "left lidar distance",
      [this] { return leftDistanceSensor.getLidarData().value(); }, nullptr);
  builder.AddDoubleProperty(
      "right lidar distance",
      [this] { return rightdistanceSensor.getLidarData().value(); }, nullptr);

  builder.AddBooleanProperty(
      "State: Align To Target", [this] { return state.alignToTarget; },
      nullptr);
  builder.AddBooleanProperty(
      "State: World Align", [this] { return state.worldAlign; }, nullptr);
}
