 ...
 * To change what motor controller runs the azimuth motor, change this to
 * either:
 * * valor::PhoenixController
 * * valor::NeoController
 */
 ...
 *
 * Subsystem responsible for driving the robot chassis, and housing all the
 * logic to control the 4 swerve modules on the robot.
 */
...
public:
   ...
   *
   * @param robot Top level robot object to parse out smart dashboard and table
   * information
   */
   ...
   *
   * Drivetrain objects have member objects on the heap - need a destructor to
   * take care of memory on destruction
   */
  ~Drivetrain();
   ...
   * * Configuring each swerve module (including controllers for azimuth and
   * drive motors)
