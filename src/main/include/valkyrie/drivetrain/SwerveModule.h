...
using meters_per_turn_t = units::unit_t<
    units::compound_unit<units::meters, units::inverse<units::turn>>>;
...
class SwerveModule
    : public wpi::Sendable,
      public wpi::SendableHelper<SwerveModule<AzimuthMotor, DriveMotor>> {
public:
  SwerveModule(AzimuthMotor *_azimuthMotor, DriveMotor *_driveMotor,
   ...
   * @param isDriveOpen true if drive should set speed using closed-loop
   * velocity control
   */
  ...
  /**
   * Command the swerve module motors to the desired state using closed-loop
   * drive speed control
   * @param desiredState the desired swerve module speed and angle
   */
  void setDesiredState(frc::SwerveModuleState desiredState) {
  ...
  /**
   * Save the current azimuth absolute encoder reference position in
   * NetworkTables preferences. Call this method following physical alignment of
   * the module wheel in its zeroed position. Used during module instantiation
   * to initialize the relative encoder.
   */

  /**
   * Loads the current azimuth absolute encoder reference position and sets
   * selected sensor encoder
  ...
  frc::Translation2d
  convertSwerveStateToVelocityVector(frc::SwerveModuleState state);
