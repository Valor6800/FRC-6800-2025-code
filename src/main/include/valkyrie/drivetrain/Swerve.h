...
public:
  ...
  Swerve(frc::TimedRobot *_robot, const char *_name,
         std::vector<std::pair<AzimuthMotor *, DriveMotor *>> modules,
         units::meter_t module_radius, units::meter_t _wheelDiameter);
  ~Swerve();
  ...
  void setupGyro(int, const char *, units::degree_t, units::degree_t,
                 units::degree_t);
  void resetGyro();
  frc::Rotation2d getGyro();

  /**
   * Reset the robot's position on the field. Any accumulted gyro drift will be
   * noted and accounted for in subsequent calls to getPoseMeters()
  ...
  getModuleStates(frc::ChassisSpeeds chassisSpeeds);
   ...
   * @param omega_radps the desired rotational velocity component in radians per
   * second
   * @param isFOC true if driving field oriented
   */
  void drive(units::velocity::meters_per_second_t vx_mps,
  ...
  getSmoothedAngularAcceleration();
  ...
  const units::radians_per_second_squared_t MAX_ROTATION_ACCEL =
      12_rad_per_s_sq;
  ...
  const frc::TrapezoidProfile<units::radian>::Constraints rot_constraints{
      MAX_ROTATION_VEL, MAX_ROTATION_ACCEL};

  const frc::TrapezoidProfile<units::meter>::Constraints y_constraints{
      MAX_Y_VEL, MAX_Y_ACCEL};
  const frc::TrapezoidProfile<units::meter>::Constraints x_constraints{
      MAX_X_VEL, MAX_X_ACCEL};
  const frc::TrapezoidProfile<units::meter>::Constraints x_constraints_l4{
      MAX_X_VEL_L4, MAX_X_ACCEL};

  void
  setXConstraints(frc::TrapezoidProfile<units::meter>::Constraints constraints);
...
protected:
  ...
  units::millimeter_t yPosTolerance = 25_mm;          // 20_mm;
  ...
  wpi::array<frc::SwerveModuleState, MODULE_COUNT> testModeDesiredStates{
      wpi::empty_array};
...
private:
  std::deque<units::angular_acceleration::radians_per_second_squared_t>
      yawRateBuffer;
  ...
  units::angular_acceleration::radians_per_second_squared_t
      angularAcceleration = 0_rad_per_s_sq;

  wpi::array<frc::SwerveModuleState, MODULE_COUNT>
  getModuleStates(units::velocity::meters_per_second_t,
                  units::velocity::meters_per_second_t,
                  units::angular_velocity::radians_per_second_t, bool);
  ...
  frc::ProfiledPIDController<units::radian> rot_controller{ROT_KP, 0.0, ROT_KD,
                                                           rot_constraints};

  frc::ProfiledPIDController<units::meter> y_controller{Y_KP, 0.0, Y_KD,
                                                        y_constraints};
  frc::ProfiledPIDController<units::meter> x_controller{X_KP, 0.0, X_KD,
                                                        x_constraints};
