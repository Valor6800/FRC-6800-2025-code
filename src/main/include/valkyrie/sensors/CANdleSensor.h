 ...
 * @link
 * https://store.ctr-electronics.com/content/user-manual/CANdle%20User's%20Guide.pdf
 * @endlink
 *
 * Animations can be set, or static colors can be set. Note that animations only
 * apply to the extra LED strips.
 *
 * Example from: @link
...
public:
   ...
   * @return RGBColor struct containing red, green, blue values matching the hex
   * code
   */
   ...
   */
  CANdleSensor(frc::TimedRobot *_robot, int _ledCount, int _segments,
               int _canID, std::string _canbus = "baseCAN");
   ...
   */
  void setAnimation(int segment, AnimationType animation, RGBColor color,
                    double speed = 1.0);
   ...
   *
   * Also responsible for clearing the appropriate memory associated with the
   * animation
...
private:
  void setAnimation(SegmentSettings *segment, AnimationType animation,
                    RGBColor color, double speed = 1.0);
