...
/**
 * @brief Sensor - debouncing and identifying rising/falling edges of boolean
 * inputs
 *
 * Class centered around debouncing boolean inputs.
 *
 * <b>What is a rising/falling edge?</b> When a boolean input changes, that is
 * called an "edge". When a boolean input goes from false to true, that is
 * called a rising edge. When a boolean input goes from true to false, that is
 * called a falling edge.
 *
 * <b>What is debouncing?</b> Debouncing is the process of filtering out edges
 * and making sure that if an edge triggers multiple times in a very quick
 * amount of time, it only counts as a single rising or falling edge.
 */
...
public:
  ...
  /**
   * @brief Setup a lambda function to detect a rising or falling edge of the
   * sensor
   *
