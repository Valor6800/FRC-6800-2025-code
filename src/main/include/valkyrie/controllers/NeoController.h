...
enum NeoControllerType { NEO, NEO_550 };

class NeoController : public BaseController<rev::spark::SparkMax> {
public:
  NeoController(valor::NeoControllerType, int _canID, valor::NeutralMode _mode,
                bool _inverted, std::string _canbus = "");

  static units::revolutions_per_minute_t
  getNeoControllerMotorSpeed(NeoControllerType controllerType) {
