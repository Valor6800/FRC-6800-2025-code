...
template <class RawOutput = ctre::phoenix6::controls::VoltageOut,
          class VelocityOutput = ctre::phoenix6::controls::VelocityVoltage,
          class PositionOutput = ctre::phoenix6::controls::MotionMagicVoltage>
class PhoenixController
    : public BaseController<ctre::phoenix6::hardware::TalonFX> {
  static_assert(
      std::is_base_of_v<ctre::phoenix6::controls::ControlRequest, RawOutput>);
  static_assert(std::is_base_of_v<ctre::phoenix6::controls::ControlRequest,
                                  VelocityOutput>);
  static_assert(std::is_base_of_v<ctre::phoenix6::controls::ControlRequest,
                                  PositionOutput>);
...
public:
  ...
  PhoenixController(valor::PhoenixControllerType controllerType, int canID,
                    valor::NeutralMode _mode, bool _inverted,
                    std::string canbus = "")
      : BaseController{new ctre::phoenix6::hardware::TalonFX(canID, canbus),
                       _inverted, _mode,
                       getPhoenixControllerMotorSpeed(controllerType)},
        req_raw_out{RawOutputUnit{}}, req_vel_out{0_tps}, req_pos_out{0_tr},
        cancoder{nullptr}, res_position(getMotor()->GetPosition()),
        res_velocity{getMotor()->GetVelocity()} {
    init();
  }

  static units::revolutions_per_minute_t
  getPhoenixControllerMotorSpeed(PhoenixControllerType controllerType) {
  ...
  void init() override {
    ...
    setCurrentLimits(STATOR_CURRENT_LIMIT, SUPPLY_CURRENT_LIMIT,
                     SUPPLY_CURRENT_THRESHOLD, SUPPLY_TIME_THRESHOLD);
    setGearRatios(rotorToSensor, sensorToMech);
    setPIDF(pidf, 0);

    wpi::SendableRegistry::AddLW(this, "PhoenixController",
                                 "ID " +
                                     std::to_string(getMotor()->GetDeviceID()));
  }
  ...
  void setCurrentLimits(units::ampere_t statorCurrentLimit,
  ...
  void setVoltageLimits(units::volt_t reverseLimit, units::volt_t forwardLimit,
  ...
  void setPositionUpdateFrequency(units::hertz_t hertz) {
    ...
    // Do we really need to set CANcoder update frequency because we always
    // access through motor controller res_position
    if (cancoder != nullptr)
  ...
  void setSpeedUpdateFrequency(units::hertz_t hertz) {
    ...
    // Do we really need to set CANcoder update frequency because we always
    // access through motor controller res_velocity
    if (cancoder != nullptr)
  ...
  void setupFollower(int canID, bool followerInverted = false) override {
    ...
    followerMotor->SetControl(
        ctre::phoenix6::controls::StrictFollower(getMotor()->GetDeviceID()));
  }

  void setPIDF(valor::PIDF _pidf, int slot = 0,
               bool saveImmediately = false) override {
    pidf = _pidf;
    ...
    if (slot == 1)
      setPIDFSlot(config.Slot1, saveImmediately);
    else if (slot == 2)
      setPIDFSlot(config.Slot2, saveImmediately);
    else
      setPIDFSlot(config.Slot0, saveImmediately); // Default case

    if (saveImmediately)
      getMotor()->GetConfigurator().Apply(config.MotionMagic);
  }

  void setupReverseHardwareLimit(
      int canID, ctre::phoenix6::signals::ReverseLimitTypeValue type,
      units::turn_t autosetPosition = 0_tr, bool saveImmediately = false) {
    ...
    config.HardwareLimitSwitch.ReverseLimitSource =
        ctre::phoenix6::signals::ReverseLimitSourceValue::RemoteCANdiS1;

    if (saveImmediately)
      getMotor()->GetConfigurator().Apply(config.HardwareLimitSwitch);
  }

  void setupForwardHardwareLimit(
      int canID, ctre::phoenix6::signals::ForwardLimitTypeValue type,
      units::turn_t autosetPosition = 0_tr, bool saveImmediately = false) {
  ...
  void setForwardLimit(units::turn_t forward,
                       bool saveImmediately = false) override {
  ...
  void setReverseLimit(units::turn_t reverse,
                       bool saveImmediately = false) override {
  ...
  void setGearRatios(double _rotorToSensor, double _sensorToMech,
                     bool saveImmediately = false) override {
  ...
  void setupCANCoder(int deviceId, units::turn_t offset, bool clockwise,
                     std::string canbus = "",
    ...
    cancoderConfig.SensorDirection =
        clockwise
  ...
  void setOpenLoopRamp(units::second_t time,
                       bool saveImmediately = false) override {
  ...
  void InitSendable(wpi::SendableBuilder &builder) override {
    ...
    builder.AddDoubleProperty(
        ...
        nullptr);
    builder.AddDoubleProperty(
        "Position", [this] { return getPosition().template to<double>(); },
        nullptr);
    builder.AddDoubleProperty(
        "Absolute Position",
        ...
        nullptr);
    builder.AddDoubleProperty(
        "Speed", [this] { return getSpeed().template to<double>(); }, nullptr);
    builder.AddDoubleProperty(
        "Out Volt",
    ...
    builder.AddDoubleProperty(
        "reqPosition",
        [this] { return req_pos_out.Position.template to<double>(); }, nullptr);
    builder.AddDoubleProperty(
        "reqSpeed",
        [this] { return req_vel_out.Velocity.template to<double>(); }, nullptr);
    builder.AddStringProperty(
        "Magnet Health", [this] { return getMagnetHealth().ToString(); },
        nullptr);
...
private:
  ...
  static constexpr units::turns_per_second_squared_t FALCON_PIDF_KA =
      130_tr_per_s_sq;
  static constexpr units::turns_per_second_cubed_t FALCON_PIDF_KJ =
      650_tr_per_s_cu;

  static constexpr units::revolutions_per_minute_t FREE_SPD_KRAKEN_X60 =
      6000_rpm;
  static constexpr units::revolutions_per_minute_t FREE_SPD_KRAKEN_X60_FOC =
      5800_rpm;
  static constexpr units::revolutions_per_minute_t FREE_SPD_KRAKEN_X44 =
      7530_rpm;
  static constexpr units::revolutions_per_minute_t FREE_SPD_KRAKEN_X44_FOC =
      7530_rpm;
  static constexpr units::revolutions_per_minute_t FREE_SPD_FALCON = 6380_rpm;
  static constexpr units::revolutions_per_minute_t FREE_SPD_FALCON_FOC =
      6080_rpm;

  template <class S> void setPIDFSlot(S &slot, bool saveImmediately) {
    ...
    if (pidf.aFF != 0) {
      slot.GravityType =
          pidf.aFFType == valor::FeedForwardType::LINEAR
    ...
    if (saveImmediately)
      getMotor()->GetConfigurator().Apply(slot);
  }

  // Helper function for enableFOC that only enables if the template has the
  // field "EnableFOC"
  template <typename T>
  static inline void tryEnableFOC(T &request, bool enableFOC) {
    constexpr bool hasEnableFOC = requires(T &t) { t.EnableFOC; };
    if constexpr (hasEnableFOC)
      request.EnableFOC = enableFOC;
  }
