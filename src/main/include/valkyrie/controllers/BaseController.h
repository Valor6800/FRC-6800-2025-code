 ...
 * season, BaseController is used to organize code and abstract a lot of the
 * base code that is often repetitive in all motor controllers.
 *
 * The idea is that motor controllers on the robot implement BaseController and
 * logic for that motor controller is run by the implemented class. Setup for
 * the motors should also occur in the implemented BaseController, and the motor
 * pointer itself also lives in this class.
 *
 * Helper methods exist to make it easier for subsystems to control motors in a
 * variety of ways.
 *
 * Usage:
 * \code {.cpp}
 ...
 */
template <class T>
class BaseController : public wpi::Sendable,
                       public wpi::SendableHelper<BaseController<T>> {
public:
  /**
   * @brief Construct a new Valor Controller object
   *
   * @param _motor The motor that will be controlled. Setup by the implemented
   * class
   */
  BaseController(T *_motor, bool _inverted, valor::NeutralMode _neutralMode,
                 units::turns_per_second_t _maxMotorSpeed)
      : maxMotorSpeed(_maxMotorSpeed),
        voltageCompenstation(units::volt_t{12.0}), motor(_motor),
        inverted(_inverted), neutralMode(_neutralMode), rotorToSensor(1),
        sensorToMech(1) {}
   ...
   * are stored on the heap so use the delete keyword to remove them from the
   * heap.
   */
  ~BaseController() {
    if (motor) {
   ...
   *
   * @return units::turns_per_second_t Motor's maximum angular speed (at the
   * rotor)
   */
   ...
   *
   * @return units::turns_per_second_t Mechanism's maximum angular speed (at the
   * mech)
   */
  units::turns_per_second_t getMaxMechSpeed() {
    return maxMotorSpeed / (rotorToSensor * sensorToMech);
  }
  ...
  void setVoltageCompensation(units::volt_t _voltageCompensation) {
    voltageCompenstation = _voltageCompensation;
  }

  units::volt_t getVoltageCompensation() { return voltageCompenstation; }

  /**
   * @brief Get a pointer to the Motor object that the BaseController
   * implementation owns
   *
   * @return T* Pointer to the motor object. Allows developers to call functions
   * attached to the motor
   */
  T *getMotor() { return motor; }

  /**
   * @brief Initialize the motor. Setup any parameters that get burned to the
   * motor's flash memory
   *
   ...
   *
   * Will use the motor's native trapezoidal motion profile to get the motor to
   * that position. Can be tuned using the velocity and acceleration components
   * of valor::PIDF via @link setPIDF @endlink
   *
   * To be defined by the implemented BaseController class
   *
   ...
   *
   * Will use the motor's native trapezoidal motion profile to get the motor to
   * that position. Can be tuned using the PIDF components of valor::PIDF via
   * @link setPIDF @endlink
   *
   * To be defined by the implemented BaseController class
   *
  ...
  /**
   * @brief If a motor is paired with another motor, setup that other motor as a
   * follower
   *
   ...
   *
   * valor::PIDF has some default values already set and tested, but if the
   * system requires some changes, use this method to change those defaults.
   *
   * @param pidf The new PIDF values to use for the system
   * @param slot Set which slot of the motor to apply the PIDF. 0 if slots
   * aren't compatible
   */
  virtual void setPIDF(valor::PIDF pidf, int slot = 0,
                       bool saveImmediately = false) = 0;
   ...
   * @param forward The forward soft limit
   */
  virtual void setForwardLimit(units::turn_t forward,
                               bool saveImmediately = false) = 0;
   ...
   * @param reverse The reverse soft limit
   */
  virtual void setReverseLimit(units::turn_t reverse,
                               bool saveImmediately = false) = 0;

  /**
   * @brief Set the gear ratios for the motor
   *
   * Used to convert between the motor's rotor and the output shaft of the
   * mechanism
   *
   * To be defined by the implemented BaseController class
   *
   * @param rotorToSensor The gear ratio from rotor to where the sensor is.
   * Should be 1 if no external sensor
   * @param sensorToMech The gear ratio from the sensor to the mechanism's
   * output shaft. Should be the gear ratio if no external sensor
   * @param saveImmediately Tell the underlying controller to apply the changes
   * immediately, or to wait until a manual apply has been called
   */
  virtual void setGearRatios(double rotorToSensor, double sensorToMech,
                             bool saveImmediately = false) = 0;
   ...
   * @param supplyTimeThreshold The amount of time before re-allowing current to
   * pass through
   */
  virtual void setCurrentLimits(units::ampere_t statorCurrentLimit,
                                units::ampere_t supplyCurrentLimit,
                                units::ampere_t supplyCurrentThreshold,
                                units::second_t supplyTimeThreshold,
                                bool saveImmediately = false) = 0;

  virtual void setNeutralMode(valor::NeutralMode mode,
                              bool saveImmediately = false) = 0;

  virtual void setOpenLoopRamp(units::second_t time,
                               bool saveImmediately = false) = 0;
  ...
  virtual void setupCANCoder(int deviceId, units::turn_t offset,
                             bool clockwise = false, std::string canbus = "",
                             units::turn_t absoluteRange = 1_tr,
                             bool saveImmediately = false) = 0;
  virtual ctre::phoenix6::hardware::CANcoder *getCANCoder() = 0;
