...
public:
  ...
  /**
   * Fills the SendableChooser with a list of .auto filenames found in
   * ~/deploy/pathplanner/autos key'd with a corresponding "human readable"
   * name.
   */
  void fillAutoList();
  ...
  /**
   * Loads and stores an auto to be retrieved with getAuto. This cuts out the
   * delay that would normally happen if the auto was generated on
   * AutonomousInit.
   *
   * Does nothing if an auto with the given filename is already loaded. Use
   * clearAutos to clear the preloaded autos list, then rebuild them.
   *
